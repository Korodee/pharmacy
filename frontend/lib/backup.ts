...
/**
 * Google Sheets cell character limit (50,000)
 * We use 49,000 to leave buffer for truncation messages
 */
...
/**
 * Connect to production MongoDB for backup
 * This ensures we're backing up production data, not localhost
 */
async function connectToProductionDatabase(): Promise<Db> {
  // Use production MongoDB URI (preferred) or fall back to MONGODB_URI
  const uri = process.env.MONGODB_URI_PROD || process.env.MONGODB_URI;

  if (!uri) {
    throw new Error(
      "MONGODB_URI or MONGODB_URI_PROD must be set. " +
        "Please configure your production MongoDB connection string in environment variables."
    );
  }

  // Prevent accidental backup of localhost
  if (
    ...
    throw new Error(
      "ERROR: Backup is attempting to connect to localhost MongoDB. " +
        "This is a safety measure to prevent backing up development data. " +
        "Please set MONGODB_URI_PROD to your production MongoDB connection string. " +
        "Current URI: " +
        uri.replace(/\/\/[^:]+:[^@]+@/, "//***:***@") // Hide credentials in error
    );
  }

  // Use MONGODB_DB environment variable if set, otherwise extract from URI, otherwise use default
  let dbName = "kateri-pharmacy";

  // Priority 1: Use MONGODB_DB environment variable if explicitly set
  if (process.env.MONGODB_DB) {
    dbName = process.env.MONGODB_DB;
  } else {
    // Priority 2: Extract database name from URI if present
    // Look for pattern: /dbname? or /dbname at the end (after the host)
    const uriParts = uri.split("?");
    const pathPart = uriParts[0];
    const pathMatch = pathPart.match(/\/([^/]+)$/);
    if (
      pathMatch &&
      pathMatch[1] &&
      !pathMatch[1].includes("@") &&
      pathMatch[1] !== "mongodb.net" &&
      pathMatch[1] !== "mongodb.com"
    ) {
  ...
  try {
    ...
    console.error("[Backup] Failed to connect to MongoDB:", error);
    throw error;
  }
}
...
export async function exportMongoCollections(): Promise<BackupData[]> {
  const db = await connectToProductionDatabase();
...
function getGoogleSheetsClient() {
  ...
  if (!credentials) {
    throw new Error(
      "GOOGLE_SERVICE_ACCOUNT_KEY is not set. Please configure it in your environment variables."
    );
  }
  ...
  try {
    ...
    throw new Error(
      "GOOGLE_SERVICE_ACCOUNT_KEY is not valid JSON. Please check your environment variable."
    );
  }
...
export async function uploadToGoogleSheets(
  ...
  try {
    ...
    const otherCollections = backupData.filter(
      (b) => b.collection !== "claims" && b.collection !== "requests"
    );
    ...
    if (claimsData && claimsData.data.length > 0) {
      const claimsByCategory = {
        medications: claimsData.data.filter(
          (c: any) => c.category === "medications"
        ),
        appeals: claimsData.data.filter((c: any) => c.category === "appeals"),
        "manual-claims": claimsData.data.filter(
          (c: any) => c.category === "manual-claims"
        ),
        "diapers-pads": claimsData.data.filter(
          (c: any) => c.category === "diapers-pads"
        ),
      };
      ...
      await writeDataToSheet(
        ...
        targetSpreadsheetId,
        "Medications",
        claimsByCategory["medications"]
      );
      await writeDataToSheet(
        sheets,
        targetSpreadsheetId,
        "Appeals",
        claimsByCategory["appeals"]
      );
      await writeDataToSheet(
        sheets,
        targetSpreadsheetId,
        "Manual Claims",
        claimsByCategory["manual-claims"]
      );
      await writeDataToSheet(
        sheets,
        targetSpreadsheetId,
        "Diapers and Pads",
        claimsByCategory["diapers-pads"]
      );
    }

    // Write requests by type
    if (requestsData && requestsData.data.length > 0) {
      const refillRequests = requestsData.data.filter(
        (r: any) => r.type === "refill"
      );
      const consultationRequests = requestsData.data.filter(
        (r: any) => r.type === "consultation"
      );
...
function formatValueForExcel(value: any): string {
    ...
    if (value.length > 0 && typeof value[0] === "object") {
      ...
      for (const item of value) {
        ...
        if (typeof item === "object") {
          // Format object as key:value pairs, but truncate each value
          itemStr = Object.entries(item)
            .map(([k, v]) => {
              const valStr = String(v);
              // Truncate individual values to prevent huge nested objects
              if (valStr.length > 1000) {
                return `${k}: ${valStr.substring(0, 1000)}...`;
              }
              return `${k}: ${valStr}`;
            })
            .join("; ");
        ...
        if (
          formatted.length + separator.length + itemStr.length >
          MAX_CELL_LENGTH
        ) {
          formatted +=
            separator +
            "[... " +
            (value.length - formatted.split(" | ").length) +
            " more items]";
          break;
        }
        formatted += separator + itemStr;
      }

      // Final truncate check
      if (formatted.length > MAX_CELL_LENGTH) {
        return (
          formatted.substring(0, MAX_CELL_LENGTH) +
          "... [TRUNCATED: " +
          formatted.length +
          " chars]"
        );
  ...
  if (typeof value === "object") {
    ...
    for (const [k, v] of Object.entries(value)) {
      ...
      // Truncate individual values to prevent huge nested objects
      if (valStr.length > 1000) {
        valStr = valStr.substring(0, 1000) + "...";
      }

      const entry = `${k}: ${valStr}`;
      const separator = formatted ? "; " : "";

      // Check if adding this entry would exceed limit
      if (
        formatted.length + separator.length + entry.length >
        MAX_CELL_LENGTH
      ) {
        formatted +=
          separator +
          "[... " +
          (Object.keys(value).length - formatted.split("; ").length) +
          " more fields]";
        break;
      }
      formatted += separator + entry;
    }
  ...
  if (typeof value === "string") {
    ...
    if (dateMatch) {
      try {
        ...
        // Not a valid date, continue to string handling
      }
    }
...
async function writeDataToSheet(
  ...
  const rows = data.map((doc: any, rowIndex: number) => {
    return headers.map((header, colIndex: number) => {
      const value = doc[header];
      let formatted = formatValueForExcel(value);

      // Final safety check - ensure no cell exceeds 50,000 characters
      if (formatted.length > 50000) {
        formatted =
          formatted.substring(0, 49000) +
          "... [TRUNCATED: " +
          formatted.length +
          " chars]";
      }
...
async function writeCollectionToSheet(
  ...
  // Convert documents to rows with formatted values (using same formatting as writeDataToSheet)
  const rows = backup.data.map((doc: any, rowIndex: number) => {
    return headers.map((header, colIndex: number) => {
      const value = doc[header];
      let formatted = formatValueForExcel(value);
...
async function createSummarySheet(
  ...
  const summarySheetId = await getSheetId(
    sheets,
    spreadsheetId,
    summarySheetName
  );
  ...
  const claimsByCategory = claimsData
    ? {
        ...
        "Diapers and Pads": claimsData.data.filter(
          (c: any) => c.category === "diapers-pads"
        ).length,
      }
    : {};
  ...
  const requestsByType = requestsData
    ? {
        "Refill Requests": requestsData.data.filter(
          (r: any) => r.type === "refill"
        ).length,
        "Consultation Requests": requestsData.data.filter(
          (r: any) => r.type === "consultation"
        ).length,
      }
    : {};
 ...
 * Exports MongoDB collections and uploads to Google Sheets
 */

...
export async function POST(request: NextRequest) {
  try {
    ...
    // Helper function to create initial status history
    ...
    // Add statusHistory to each claim
    const claimsWithHistory = dummyClaims.map((claim) => ({
      ...claim,
    ...
    // Insert dummy claims
    await collection.insertMany(claimsWithHistory);

    return NextResponse.json({
      success: true,
      message: `Successfully seeded ${claimsWithHistory.length} dummy claims`,
      claims: claimsWithHistory,
    });
  } catch (error) {

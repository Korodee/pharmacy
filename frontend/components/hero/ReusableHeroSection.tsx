...
export default function ReusableHeroSection({ 
  ...
  const handleButtonClick = (button: any) => {
    if (button.text === "Request A Refill") {
      setIsModalOpen(true);
    } else if (button.text === "Book a Consultation") {
      setIsConsultationModalOpen(true);
    } else if (button.href && button.href.startsWith('#')) {
      // Handle anchor links
  ...
  return (
    ...
    >
      ...
      {/* Background Overlay */}
      <div className="absolute inset-0 bg-[#0A438C]"></div>
          ...
          <div className="min-h-[80vh] flex items-center justify-center">
            ...
            <div className="flex items-center justify-center w-full">
              <motion.div
                ...
                initial="hidden"
                animate="visible"
                className="text-white text-center max-w-6xl mx-auto px-8 py-28"
              >
                <motion.h1 
                  variants={fadeUp}
                  className="text-4xl md:text-6xl font-medium leading-tight mb-5"
                >
                  {content.primaryHeading}
                </motion.h1>
                <motion.p 
                  variants={fadeUp}
                  className="text-xl md:text-2xl font-light text-[#D4F4FF] mb-8 leading-relaxed max-w-4xl mx-auto"
                >
                  {content.description}
                </motion.p>
                <motion.div
                ...
                >
                  {content.buttons.map((button, index) => (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      className={`px-8 py-4 rounded-xl font-semibold text-lg transition-all duration-200 shadow-lg hover:shadow-xl ${
                        button.variant === 'primary'
                          ? 'bg-white text-[#0A438C] hover:bg-white/90'
                          : 'bg-[#0A438C] text-white hover:bg-[#0A438C]/90 border-2 border-white'
                      }`}
                    >
                      {button.text}
                    </motion.button>

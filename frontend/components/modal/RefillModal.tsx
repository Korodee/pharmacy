...
export default function RefillModal({ isOpen, onClose }: RefillModalProps) {
  ...
  const getAvailableTimes = (selectedDate?: string) => {
    ...
    if (selectedDate) {
      ...
      // If it's today, start from 4 hours from now (or current time if it's past business hours)
      if (isToday) {
        const fourHoursLater = new Date(now);
        fourHoursLater.setHours(fourHoursLater.getHours() + 4);
        
        // If 4 hours later is still within business hours, use that as start time
        if (fourHoursLater.getHours() < businessEnd || 
            (fourHoursLater.getHours() === Math.floor(businessEnd) && 
             fourHoursLater.getMinutes() <= (businessEnd - Math.floor(businessEnd)) * 60)) {
